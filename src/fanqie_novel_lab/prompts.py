...
- 节奏快，前三章有强钩子，但逻辑自洽。
- 降低 AI 味：少用模板化套话、空泛总结和工整排比，多用具体物件、人物动作、差异化对白。
- 反转要出乎意料但有迹可循：每个关键反转必须来自已埋伏笔、人物动机、规则限制或利益关系。
- 输出必须是 JSON，字段必须完整。
""".strip()
...
- first_10_chapters: 10 章，每章包含 chapter, title, goal, conflict, twist, hook_type, foreshadowing, reversal_logic, ending_hook
- long_arc: 8-12 条长期伏笔/成长线
- recurring_hooks: 10-16 条可复用章节钩子，必须包含身份误判、利益倒挂、规则漏洞、关系背刺、证据反咬、倒计时、代价兑现、隐藏盟友、道德两难、承诺反噬等不同类型
- risk_notes: 5-8 条风险提醒，特别是撞梗、节奏、AI味
- revision_notes: 空数组

额外限制：
1. 不要把钩子写成空泛悬念；每个钩子必须落到具体证据、选择、物件、人物动作或代价。
2. 每个 twist 都要写清 foreshadowing：至少 1-2 个前文可回看的线索。
3. reversal_logic 必须说明反转为什么合理，不能靠临时新增规则、角色降智或空降人物。
4. risk_notes 必须包含“AI味风险”和“硬反转风险”的修正建议。
""".strip()
...
4. 强化反派压迫感和阶段性目标。
5. 不仿写、不靠近任何具体作品。
6. 降低 AI 味：删除模板化套话，增加具体动作、物件、人物口吻和职业细节。
7. 强化有迹可循的反转：补足低调伏笔、误导点和反转后的因果解释。
8. 在 revision_notes 中逐条说明你修改了什么。
...
- 保持人物、金手指规则、伏笔与大纲一致。
- 降低 AI 味：不要模板化解释、不要空泛总结、不要所有角色同一种说话方式。
- 反转必须有迹可循：先给低调线索，再改变读者对证据/人物/规则的理解。
...
6. 避免 AI 味：少用“全场死寂、命运齿轮、他不知道的是、震惊所有人”等套话；用具体动作、物件、对白潜台词呈现情绪。
7. 本章反转必须提前埋线索，反转后让读者能回看发现合理依据。
8. 章末钩子必须具体，优先使用证据反咬、身份误判、代价兑现、关系背刺、倒计时或规则漏洞。
""".strip()
...
- 本片段必须能与前文自然衔接。
- 避免 AI 味和空泛套话；用可见细节与人物口吻推进。
- 如果写反转，必须让线索提前自然出现，不能硬拐。
""".strip()
...
7. 不要用“他不知道的是、全场死寂、命运齿轮”等模板句；钩子落在具体证据、选择、物件或人物动作上。
8. 反转前至少埋一个低调线索，反转后让线索意义变化。
""".strip()
